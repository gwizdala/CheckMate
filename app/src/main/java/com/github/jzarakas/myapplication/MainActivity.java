...
import com.github.jzarakas.myapplication.OnServerRequestCompleteListener;
                ...
                for (int i = 0; i < count; i++) {
                    ...
                    if (count > 0) {
                        ...
                        for (int n = 0; n < count; n++) {
                            ...
                            System.out.println(texts.getString("text"));
                            //remove white space and switch characters
                            text = text + texts.getString("text").replaceAll("\\s+", "").toLowerCase();
                            text = text.replaceAll("l", "1");
                            text = text.replaceAll("o", "0");
                        }

                        if (text.contains("t0ta1")) {
                            text = text.substring(text.lastIndexOf("t0ta1") + 5);
                            ...
                            while(decimalCount < 2 && iterator < text.length()){
                                if(Character.isDigit(text.charAt(iterator))) {
                                    if(isDecimal){
                                        decimalCount++;
                                    }
                                    totalFloat = totalFloat + text.charAt(iterator);
                                }
                            ...
                            Scanner scanner = new Scanner(totalFloat);
                            ...
                            Toast.makeText(this, "Could not find total", Toast.LENGTH_LONG).show();
                        
                        Intent myIntent = new Intent(getApplicationContext(), Processed.class);
                        myIntent.putExtra("checkResult", total);
